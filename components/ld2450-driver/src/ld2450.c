...
esp_err_t ld2450_init(const ld2450_config_t *config)
{
    ...
    // Install UART driver
    ret = uart_driver_install(config->uart_port, CONFIG_LD2450_UART_RX_BUF_SIZE * 2, 
    ...
    ESP_LOGI(TAG, "LD2450 driver initialized on UART%d (RX: GPIO%d, TX: GPIO%d, baud: %d)", 
             config->uart_port, config->uart_rx_pin, config->uart_tx_pin, config->uart_baud_rate);
    
    // Start processing task if auto-processing is enabled
    if (config->auto_processing) {
        xTaskCreate(ld2450_processing_task, "ld2450_task", CONFIG_LD2450_TASK_STACK_SIZE,
